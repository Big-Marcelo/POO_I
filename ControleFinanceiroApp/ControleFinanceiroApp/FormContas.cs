...
namespace ControleFinanceiroApp
{
    public partial class FormContas : Form
    {
        ...
        private void AdicionarConta(int idUsuario, string tipo, string banco, string numero, decimal saldo)
        {
            using var conn = new NpgsqlConnection(connStr);
            conn.Open();
            var cmd = new NpgsqlCommand("INSERT INTO conta (id_usuario, tipo, banco, numero, saldo) VALUES (@u, @t, @b, @n, @s)", conn);
        ...
        private void btnAdicionarConta_Click(object sender, EventArgs e)
        {
            ...
            AdicionarConta(idUsuario, tipoSelecionado, banco, numero, saldo);
            CarregarContas();
        }
